...
public class MetricsPublisherImpl implements MetricsPublisher {
    ...
    public void publishExceptionMetric(final Instant timestamp, final Action action, final Throwable e) {
        ...
        publishMetric(Metrics.METRIC_NAME_HANDLER_EXCEPTION, dimensions, StandardUnit.COUNT, 1.0, timestamp);
    }
    ...
    public void publishInvocationMetric(final Instant timestamp, final Action action) {
        ...
        publishMetric(Metrics.METRIC_NAME_HANDLER_INVOCATION_COUNT, dimensions, StandardUnit.COUNT, 1.0, timestamp);
    }
    ...
    public void publishDurationMetric(final Instant timestamp, final Action action, final long milliseconds) {
        ...
        publishMetric(Metrics.METRIC_NAME_HANDLER_DURATION, dimensions, StandardUnit.MILLISECONDS, (double) milliseconds,
            timestamp);
    }
    ...
    private void publishMetric(final String metricName,
        ...
        for (final Map.Entry<String, String> kvp : dimensionData.entrySet()) {
            final Dimension dimension = Dimension.builder().name(kvp.getKey()).value(kvp.getValue()).build();
            dimensions.add(dimension);
        }

        final MetricDatum metricDatum = MetricDatum.builder().metricName(metricName).unit(unit).value(value)
            .dimensions(dimensions).timestamp(timestamp).build();

        final PutMetricDataRequest putMetricDataRequest = PutMetricDataRequest.builder()
            .namespace(String.format("%s/%s", Metrics.METRIC_NAMESPACE_ROOT, resourceNamespace)).metricData(metricDatum).build();
