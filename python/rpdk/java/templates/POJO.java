...
public class {{ pojo_name|uppercase_first_letter }} {
    ...
    public JSONObject getPrimaryIdentifier(){
        ...
        {% set components = identifier.split("/") %}
        if (this.get{{components[2]}}() != null{% for i in range(4, components|length + 1) %} && this{% for component in components[2:i] %}.get{{component|uppercase_first_letter}}(){% endfor %} != null{% endfor %}) {
